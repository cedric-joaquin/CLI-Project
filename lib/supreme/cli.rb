class Supreme::CLI
    ...
    def initialize
        ...
        Supreme::Scraper.scrape_categories
        @session = Supreme::Cart.new
        @status = "shopping"
    end
    ...
    def add_to_cart
        ...
        while !input.between?(1,Supreme::Category.all.size)
            puts "\nInvalid selection, please select a valid category."
            input = gets.chomp.to_i
        end
        category = Supreme::Category.select(Supreme::CLI.to_index(input))
        
        #Product Selection
        Supreme::Scraper.scrape_products(category) unless Supreme::Product.all.collect{|prod| prod.category}.include? (category)
        if Supreme::Product.products_by_category(category).empty?
            ...
            input = gets.chomp.to_i
            while !input.between?(1,Supreme::Product.products_by_category(category).size)
                puts "\nInvalid selection, please select a valid product."
                input = gets.chomp.to_i
            end
            product = Supreme::Product.select_by_category(category,Supreme::CLI.to_index(input))
            ...
            style = product.select_style(Supreme::CLI.to_index(input))
            ...
            size = product.styles.find{|a|a.keys[0] == style}[style][Supreme::CLI.to_index(input)]
    ...
    def display_categories
        puts "\nCategories:"
        Supreme::Category.all.each_with_index {|cat, i| puts "  #{i+1}. #{cat.name}"}
    end

    def display_products(category)
        puts "\nAvailable Products in #{category.capitalize}:"
        Supreme::Product.all.select{|a| a.category == category}.each_with_index do |prod, i|
    ...
    def display_styles(product)
        ...
        Supreme::Product.all.select{|a| a.name == product.name}.each do |prod|
